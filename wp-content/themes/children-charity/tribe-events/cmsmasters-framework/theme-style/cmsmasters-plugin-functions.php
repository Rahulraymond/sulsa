...
function children_charity_theme_color( $color ) {
	$cmsmasters_option = children_charity_get_global_options();

	$colors = array(
		'color' => $cmsmasters_option['children-charity_default_color'],
		'link' => $cmsmasters_option['children-charity_default_link'],
		'hover' => $cmsmasters_option['children-charity_default_hover'],
		'heading' => $cmsmasters_option['children-charity_default_heading'],
		'bg' => $cmsmasters_option['children-charity_default_bg'],
		'alternate' => $cmsmasters_option['children-charity_default_alternate'],
		'border' => $cmsmasters_option['children-charity_default_border'],
		'secondary' => $cmsmasters_option['children-charity_default_secondary'],
	);
...
function children_charity_hex_color( $color ) {
	$old_color = children_charity_theme_color( $color );
...
function children_charity_tribe_update_default_options() {
	...
	$new_global_elements = array(
		'event_title_color' => children_charity_hex_color( 'heading' ),
		'event_date_time_color' => children_charity_hex_color( 'color' ),
		'link_color' => children_charity_hex_color( 'link' ),
		'background_color_choice' => 'transparent',
		'background_color' => 'transparent',
		'accent_color' => children_charity_hex_color( 'link' ),
		'map_pin' => '',
	);
	...
	$new_single_event = array(
		...
		'post_title_color' => children_charity_hex_color( 'heading' ),
	...
	$new_tec_events_bar = array(
		'events_bar_text_color' => children_charity_hex_color( 'heading' ),
		'find_events_button_text_color' => children_charity_hex_color( 'bg' ),
		'events_bar_icon_color_choice' => 'default',
		'events_bar_icon_color' => children_charity_hex_color( 'heading' ),
		'find_events_button_color_choice' => 'custom',
		'find_events_button_color' => children_charity_hex_color( 'link' ),
		'events_bar_background_color_choice' => 'default',
		'events_bar_background_color' => children_charity_hex_color( 'bg' ),
		'view_selector_background_color_choice' => 'default',
		'view_selector_background_color' => children_charity_hex_color( 'bg' ),
		'events_bar_border_color_choice' => 'default',
		'events_bar_border_color' => children_charity_hex_color( 'border' ),
	);
	...
	$new_month_view = array(
		'days_of_week_color' => children_charity_hex_color( 'heading' ),
		'date_marker_color' => children_charity_hex_color( 'heading' ),
		'multiday_event_bar_color_choice' => 'default',
		'multiday_event_bar_color' => '#334aff',
		'grid_lines_color' => children_charity_hex_color( 'border' ),
		'grid_hover_color' => children_charity_hex_color( 'border' ),
		'grid_background_color_choice' => 'transparent',
		'grid_background_color' => children_charity_hex_color( 'color' ),
...
add_action( 'init', 'children_charity_tribe_update_default_options' );


/* The Events Calendar and related plugins: Add your own location for template file loading. */
function children_charity_tribe_additional_template_locations( string $file, string $template ) {
	$new_locations = array();

	if ( ! tribe_events_views_v2_is_enabled() ) {
		$new_locations['themes_root'] = trailingslashit( get_theme_root() ) . 'children-charity/tribe-events';
	}
...
add_filter( 'tribe_events_template', 'children_charity_tribe_additional_template_locations', 10, 2 );
...
function children_charity_tribe_events_register_styles_scripts() {
	// Styles
	if ( tribe_events_views_v2_is_enabled() ) {
		wp_enqueue_style( 'children-charity-tribe-events-v2-style', get_template_directory_uri() . '/tribe-events/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-v2-style.css', array(), '1.0.0', 'screen' );
	} else {
...
function children_charity_tribe_events_layout($cmsmasters_layout) {
	if (
...
add_filter('cmsmasters_theme_page_layout_filter', 'children_charity_tribe_events_layout');
