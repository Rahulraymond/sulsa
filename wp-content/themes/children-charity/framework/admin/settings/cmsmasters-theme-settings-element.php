...
function children_charity_options_element_fields($set_tab = false) {
	...
	case 'code':
		...
		$options[] = array( 
			'section' => 'code_section', 
			'id' => 'children-charity' . '_twitter_access_data', 
			'title' => esc_html__('Twitter Access Data', 'children-charity'), 
			'desc' => sprintf(
				/* translators: Twitter access data. %s: Link to twitter access data generator */
				esc_html__( 'Generate %s and paste access data to fields.', 'children-charity' ),
				'<a href="' . esc_url( 'https://api.cmsmasters.net/wp-json/cmsmasters-api/v1/twitter-request-token' ) . '" target="_blank">' .
					esc_html__( 'twitter access data', 'children-charity' ) .
				'</a>'
			), 
			'type' => 'multi-text', 
			'std' => $defaults[$tab]['children-charity' . '_twitter_access_data'], 
			'class' => 'regular-text', 
			'choices' => array( 
				esc_html__('Consumer Key', 'children-charity') . '|consumer_key', 
				esc_html__('Consumer Secret', 'children-charity') . '|consumer_secret', 
				esc_html__('Access Token', 'children-charity') . '|access_token', 
				esc_html__('Access Token Secret', 'children-charity') . '|access_token_secret' 
			) 
		);
