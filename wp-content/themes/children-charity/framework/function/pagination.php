...
function cmsmasters_pagination($max_num_pages = NULL, $shortcode_id = '') {
	...
	$pagination = array( 
		...
		'mid_size' => 1, 
		'prev_next' => true, 
		'prev_text' => '<span class="cmsmasters_theme_icon_pagination_' . (!is_rtl() ? 'prev' : 'next') . '"><span>' . esc_html(apply_filters('children_charity_pagination_prev_text_filter', '')) . '</span></span>', 
		'next_text' => '<span class="cmsmasters_theme_icon_pagination_' . (!is_rtl() ? 'next' : 'prev') . '"><span>' . esc_html(apply_filters('children_charity_pagination_next_text_filter', '')) . '</span></span>', 
		'type' => 'list', 
		'add_args' => false, 
		'add_fragment' => '' 
	);
